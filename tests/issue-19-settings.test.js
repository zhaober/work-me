...
describe('Issue-19 设置默认值（修复「我的」界面不跳转）', () => {
  it('getDefaultSettings 返回完整默认结构', () => {
    assert.deepStrictEqual(getDefaultSettings(), {
      theme: 'light', soundOn: true, style: 'classic', bgImage: null,
    });
  });
  ...
  it('normalizeSettings 保留合法字段', () => {
    const r = normalizeSettings({ theme: 'dark', soundOn: false, style: 'depth', bgImage: 'data:abc' });
    assert.deepStrictEqual(r, { theme: 'dark', soundOn: false, style: 'depth', bgImage: 'data:abc' });
  });
